...
<?= $this->section('title') ?>Dashboard Universitas<?= $this->endSection() ?>
...
// ── Masalah Utama (sorted desc dari controller) ───────────────────────────────
...
<div class="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-2">
  <div>
    <h4 class="fw-bold mb-1" style="color:#1a2b40;">Dashboard Universitas</h4>
...
<div class="d-flex align-items-start gap-2 mb-3 px-3 py-2 rounded" style="background:rgba(13,110,253,.06);border-left:3px solid rgba(13,110,253,.3);font-size:.8rem;color:#444;">
  ...
  <span>
    <strong>Sesi</strong> — total kunjungan konseling yang tercatat; satu mahasiswa dapat memiliki lebih dari satu sesi.&ensp; <br>
  ...
  <div class="card-body py-3">
    <form method="GET" id="filterForm" class="row g-2 align-items-end">
      ...
      <div class="col-sm-6 col-md-4 col-xl-3">
        <label class="form-label form-label-sm mb-1" style="color:#666;">Fakultas</label>
        <select name="fakultas" id="filterFakultas" class="form-select form-select-sm" style="font-size:.8rem;">
          <option value="">Semua Fakultas</option>
          <?php foreach ($fakultasMap as $fid => $fnama): ?>
            <option value="<?= esc($fid) ?>" <?= ($filters['fakultas'] === $fid) ? 'selected' : '' ?>>
              <?= esc($fnama) ?>
            </option>
          <?php endforeach ?>
        </select>
      ...
      <!-- Program Studi (di-cascade oleh Fakultas) -->
      <div class="col-sm-6 col-md-4 col-xl-3">
        <label class="form-label form-label-sm mb-1" style="color:#666;">Program Studi</label>
        <select name="lembaga" id="filterLembaga" class="form-select form-select-sm" style="font-size:.8rem;">
          <option value="">Semua Prodi</option>
          <?php foreach ($allLembaga as $row): ?>
            <option value="<?= esc($row['id_lembaga']) ?>"
              data-fak="<?= esc($row['id_fakultas']) ?>"
              <?= ($filters['lembaga'] === $row['id_lembaga']) ? 'selected' : '' ?>>
              <?= esc($row['nama_prodi']) ?>
            </option>
          <?php endforeach ?>
        </select>
      ...
      <div class="col-sm-6 col-md-3 col-xl-2">
        <label class="form-label form-label-sm mb-1" style="color:#666;">Semester</label>
        <select name="smt_akd" class="form-select form-select-sm filter-auto" style="font-size:.8rem;">
          <option value="" <?= empty($filters['smt_akd']) ? 'selected' : '' ?>>Semua</option>
      ...
      <div class="col-sm-6 col-md-3 col-xl-1 d-flex gap-2 align-self-end flex-wrap">
        <button type="submit" class="btn btn-sm btn-primary flex-grow-1" style="font-size:.8rem;">
          <i class="ti tabler-filter me-1"></i>Terapkan
        </button>
        <a href="<?= base_url('admin/dashboard-univ') ?>" class="btn btn-sm btn-outline-secondary" title="Reset filter" style="font-size:.8rem;">
          <i class="ti tabler-refresh"></i>
        </a>
        <a id="btnExportPdf" href="#" target="_blank"
           class="btn btn-sm btn-outline-danger" title="Export PDF" style="font-size:.8rem;"
           onclick="this.href='<?= base_url('admin/dashboard-univ/pdf') ?>?'+new URLSearchParams(new FormData(document.getElementById('filterForm'))).toString()">
          ...
          <table class="table table-sm mb-0" style="font-size:.8rem;">
            ...
            <tbody>
              ...
              <tr class="table-light fw-bold">
                <td>Total</td>
                <td class="text-end"><?= $jkMhsTotal ?></td>
...
<div class="card border-0 shadow-sm mb-4">
  ...
  <div class="card-body px-4">
    <div class="row g-3">
      <div class="col-md-12">
        <canvas id="chartMasalah" height="100"></canvas>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  new Chart(document.getElementById('chartJk'), {
    ...
    data: {
      ...
      datasets: [{
        data: <?= json_encode($jkValues) ?>,
        backgroundColor: <?= json_encode($jkColors) ?>,
        hoverOffset: 4
      }]
    },
    options: {
  ...
  new Chart(document.getElementById('chartStatusKons'), {
    ...
    data: {
      ...
      datasets: [{
        data: <?= json_encode($statusKonsValues) ?>,
        backgroundColor: <?= json_encode($statusKonsColors) ?>,
        hoverOffset: 4
      }]
    },
    options: {
  ...
  function cascadeProdi(selectedFak) {
    filterLembaga.querySelectorAll('option[data-fak]').forEach(opt => {
      const match = !selectedFak || opt.dataset.fak === selectedFak;
      opt.hidden = !match;
  ...
  // Terapkan cascade saat halaman dimuat (saat ada filter aktif)
  cascadeProdi(filterFakultas.value);

  // Ganti fakultas → reset prodi → cascade → submit
  ...
  // Ganti prodi → langsung submit
  filterLembaga.addEventListener('change', () => filterForm.submit());

  // Filter lain (tahun, semester, jk, tanggal) → auto-submit
  ...
  document.querySelectorAll('.stressor-detail-link').forEach(el => {
    el.addEventListener('click', function (e) {
      ...
      fetch('<?= base_url('admin/stressor-detail') ?>?' + params.toString())
        .then(r => r.json())
        .then(data => {
