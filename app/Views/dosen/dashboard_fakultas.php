...
<?= $this->section('title') ?>Dashboard Fakultas<?= $this->endSection() ?>
...
<div class="d-flex align-items-start justify-content-between mb-4 flex-wrap gap-2">
  <div>
    <h4 class="fw-bold mb-1" style="color:#1a2b40;">Dashboard Fakultas</h4>
    <p class="text-muted mb-0" style="font-size:.875rem;">
      <?= esc($fakultas) ?>
      <?php if (! empty($filters['prodi']) && isset($prodiNameMap[$filters['prodi']])): ?>
        <span class="mx-1 text-muted">—</span>
        <span style="color:#1a5f7a;"><?= esc($prodiNameMap[$filters['prodi']]) ?></span>
      <?php endif ?>
    </p>
  </div>
    ...
    <form method="GET" id="filterForm">
      <div class="row g-2 align-items-end">
        ...
        <div class="col-sm-12 col-md-4 col-xl-3">
          ...
          <select name="prodi" class="form-select form-select-sm filter-auto" style="font-size:.8rem;">
            <option value="">Semua Prodi</option>
            <?php foreach ($prodiList as $p): ?>
              <option value="<?= esc($p['id_lembaga']) ?>" <?= ($filters['prodi'] === $p['id_lembaga']) ? 'selected' : '' ?>>
                <?= esc($p['nama_prodi']) ?>
              </option>
            <?php endforeach ?>
          </select>
        ...
        <div class="col-sm-6 col-md-3 col-xl-1">
          <label class="form-label form-label-sm mb-1" style="color:#666;">JK</label>
      ...
      <div class="d-flex gap-2 justify-content-end mt-2">
        <button type="submit" class="btn btn-sm btn-primary" style="font-size:.8rem;">
          <i class="ti tabler-filter me-1"></i>Filter
        </button>
        <a href="<?= base_url('dosen/dashboard-fakultas') ?>" class="btn btn-sm btn-outline-secondary" title="Reset filter" style="font-size:.8rem;">
          <i class="ti tabler-refresh"></i>
        </a>
        <a href="#" target="_blank" class="btn btn-sm btn-outline-danger" title="Export PDF" style="font-size:.8rem;"
           onclick="this.href='<?= base_url('dosen/dashboard-fakultas/pdf') ?>?'+new URLSearchParams(new FormData(document.getElementById('filterForm'))).toString()">
...
<div class="card border-0 shadow-sm mb-4">
  <div class="card-header bg-transparent border-0 pb-0 pt-3 px-4">
    <h6 class="fw-semibold mb-0" style="color:#1a2b40;">
      <i class="ti tabler-building-community me-1 text-primary"></i>Program Studi dalam Fakultas
    </h6>
  </div>
  <div class="card-body px-4 pb-3">
    <div class="d-flex flex-wrap gap-2">
      <?php foreach ($prodiList as $prodi): ?>
      <span class="badge rounded-pill px-3 py-2" style="background:rgba(26,95,122,.08);color:#1a5f7a;font-size:.78rem;font-weight:500;">
...
document.querySelectorAll('.stressor-detail-link').forEach(el => {
  el.addEventListener('click', function (e) {
    ...
    fetch('<?= base_url('dosen/stressor-detail-fakultas') ?>?' + params.toString())
      .then(r => r.json())
      .then(data => {
